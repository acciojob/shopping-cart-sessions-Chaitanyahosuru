...
function renderCart() {
  ...
  cart.forEach((item) => {
    ...
    const li = document.createElement("li");
    li.innerHTML = `${product.name} - $${product.price} <button class="remove-from-cart-btn" data-id="${(link unavailable)}">Remove</button>`;
...
function addToCart(productId) {
  if (!cart.find((item) => (link unavailable) === productId)) {
...
function removeFromCart(productId) {
  cart = cart.filter((item) => (link unavailable) !== productId);
  sessionStorage.setItem("cart", JSON.stringify(cart));
  renderCart();
}
